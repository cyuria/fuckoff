import pytest
from fuckoff.rules.gradle_wrapper import match, get_new_command
from fuckoff.types import Command


@pytest.fixture(autouse=True)
def exists(mocker):
    return mocker.patch('fuckoff.rules.gradle_wrapper.os.path.isfile',
                        return_value=True)
...
def test_match(mocker, command):
    mocker.patch('fuckoff.rules.gradle_wrapper.which', return_value=None)
...
def test_not_match(mocker, exists, command, gradlew, which):
    mocker.patch('fuckoff.rules.gradle_wrapper.which', return_value=which)
