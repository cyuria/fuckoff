...
def patch_get_key(monkeypatch):
    def patch(vals):
        vals = iter(vals)
        monkeypatch.setattr('fuckoff.ui.get_key', lambda: next(vals))
...
def test_command_selector():
    selector = ui.CommandSelector([
        CorrectedCommand('1'),
        CorrectedCommand('2'),
        CorrectedCommand('3'),
    ])
    assert selector.value.script == '1'
    selector.next()
    assert selector.value.script == '2'
    selector.next()
    assert selector.value.script == '3'
    selector.next()
    assert selector.value.script == '1'
    selector.previous()
    assert selector.value.script == '3'
...
class TestSelectCommand(object):
    ...
    def test_without_commands(self, capsys):
        assert ui.select_command(_ for _ in []) is None
        assert capsys.readouterr() == ('', 'No fucks given\n')

    def test_without_confirmation(self, capsys, commands, settings):
        settings.require_confirmation = False
        assert ui.select_command(command for command in commands) == commands[0]
    ...
    def test_without_confirmation_with_side_effects(
        ...
        settings.require_confirmation = False
        assert (ui.select_command(command for command in commands_with_side_effect)
    ...
    def test_with_confirmation_abort(self, capsys, patch_get_key, commands):
        ...
        assert ui.select_command(command for command in commands) is None
        assert capsys.readouterr() == (
            '', const.USER_COMMAND_MARK + u'\x1b[1K\rls [enter/↑/↓/ctrl+c]\nAborted\n')
    ...
    def test_with_confirmation_select_second(self, capsys, patch_get_key, commands):
        ...
        assert ui.select_command(command for command in commands) == commands[1]
        stderr = (
