...
def get_aliases(mocker):
    mocker.patch('fuckoff.shells.shell.get_aliases',
...
def test_get_all_executables_pathsep(path, pathsep):
    with patch('fuckoff.utils.Path') as Path_mock:
        get_all_executables()
...
class TestCache(object):
    ...
    def shelve(self, mocker):
        ...
        mocker.patch('fuckoff.utils.shelve.open', new_callable=lambda: _Shelve)
        return value

    @pytest.fixture(autouse=True)
    def enable_cache(self, monkeypatch, shelve):
        _ = shelve
        monkeypatch.setattr('fuckoff.utils._cache_disabled', False)
        _cache.__init__()

    @pytest.fixture(autouse=True)
    def mtime(self, mocker):
        mocker.patch('fuckoff.utils.os.path.getmtime', return_value=0)
    ...
    def key(self, monkeypatch):
        monkeypatch.setattr('fuckoff.utils.Cache._get_key',
                            lambda *_: 'key')
        return 'key'
...
class TestGetValidHistoryWithoutCurrent(object):
    ...
    def history(self, mocker):
        mock = mocker.patch('fuckoff.shells.shell.get_history')
    ...
    def alias(self, mocker):
        return mocker.patch('fuckoff.utils.get_alias',
                            return_value='fuck')
    ...
    def bins(self, mocker):
        ...
        return mocker.patch('fuckoff.utils.Path', return_value=path_mock)
